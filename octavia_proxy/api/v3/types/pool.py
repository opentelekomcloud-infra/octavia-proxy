...
class SlowStartResponse(types.BaseType):
    """Defines which attributes are to be shown on any response."""
    enable = wtypes.wsattr(bool)
    duration = wtypes.wsattr(wtypes.IntegerType())


class SlowStartPOST(types.BaseType):
    """Defines mandatory and optional attributes of a POST request."""
    enable = wtypes.wsattr(
        bool,
        default=False,
        mandatory=True)
    duration = wtypes.wsattr(
        wtypes.IntegerType(minimum=30, maximum=1200),
        default=30,
        mandatory=True)


class SlowStartPUT(types.BaseType):
    """Defines attributes that are acceptable of a PUT request."""
    enable = wtypes.wsattr(
        bool,
        default=False)
...
class PoolResponse(BasePoolType):
    ...
    slow_start = wtypes.wsattr(SlowStartResponse)
...
class PoolPOST(BasePoolType):
    ...
    slow_start = wtypes.wsattr(SlowStartPOST)
